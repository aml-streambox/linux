...
int meson_gpu_data_invalid_count;
int meson_gpu_fault;

int __weak kbase_platform_power_init_before_clocks(struct kbase_device *kbdev)
{
	return 0;
}

void __weak kbase_platform_power_term_after_clocks(struct kbase_device *kbdev)
{
}
...
struct clk *__weak kbase_platform_get_clock(struct kbase_device *kbdev,
					    unsigned int index)
{
	return of_clk_get(kbdev->dev->of_node, index);
}
...
int power_control_init(struct kbase_device *kbdev)
{
	...
	int err = 0;
	unsigned int i;
	bool platform_power_prepared = false;
#if defined(CONFIG_REGULATOR)
	static const char * const regulator_names[] = {
		"mali", "shadercores", NULL
	};
	BUILD_BUG_ON((ARRAY_SIZE(regulator_names) - 1) < BASE_MAX_NR_CLOCKS_REGULATORS);
#if defined(CONFIG_PM_OPP) && (KERNEL_VERSION(6, 0, 0) <= LINUX_VERSION_CODE)
	const char *opp_regulator_names[BASE_MAX_NR_CLOCKS_REGULATORS + 1] = {};
#endif
...
#if defined(CONFIG_PM_OPP) && (KERNEL_VERSION(6, 0, 0) <= LINUX_VERSION_CODE)
		opp_regulator_names[i] = regulator_names[i];
#endif
	}
	if (err == -EPROBE_DEFER) {
		while (i > 0)
...
#endif
	err = kbase_platform_power_init_before_clocks(kbdev);
	if (err)
		goto clocks_probe_defer;
	platform_power_prepared = true;
	 ...
	 */
	for (i = 0; i < BASE_MAX_NR_CLOCKS_REGULATORS; i++) {
		kbdev->clocks[i] = kbase_platform_get_clock(kbdev, i);
	...
	if (kbdev->nr_regulators > 0) {
		kbdev->token = dev_pm_opp_set_regulators(kbdev->dev, opp_regulator_names);
...
regulators_probe_defer:
	...
	if (platform_power_prepared)
		kbase_platform_power_term_after_clocks(kbdev);
#endif
...
int kbase_device_coherency_init(struct kbase_device *kbdev)
{
	...
	dma_coherent = of_property_read_bool(kbdev->dev->of_node, "dma-coherent");
