...
{
	resets = devm_reset_control_array_get_optional_exclusive(kbdev->dev);
	if (IS_ERR(resets)) {
		int err = PTR_ERR(resets);
...
int kbase_platform_power_init_before_clocks(struct kbase_device *kbdev)
{
	int ret;
...
struct clk *kbase_platform_get_clock(struct kbase_device *kbdev,
				     unsigned int index)
{
	if (index == 0)
		return clk_get(kbdev->dev, NULL);

	return ERR_PTR(-ENOENT);
}

void kbase_platform_power_term_after_clocks(struct kbase_device *kbdev)
{
	if (!resets)
		return;
